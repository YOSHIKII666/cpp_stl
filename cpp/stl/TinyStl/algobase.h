...
namespace mystl {
    ...
    template<class Tp,class Up>
    typename std::enable_if<std::is_same<typename std::remove_const<Tp>::type ,Up>::value&&std::is_trivially_assignable<Up>::value,*Up>::type
    ...
    //copy_if,把[first,last)中满足UnaryPredicate关系的元素拷贝到result为起点的位置上
    ...
    //move,把[first,last)区间内元素移动到[result,result+(last-first))中
    template<class InputIter,class OutputIter>
    ...
    //move_backward,将[first,last)区间内元素移动到[result-(last-first),result]区间中
    template<class BidirectionalIter1,class BidirectionalIter2>
    ...
    //fill,为[first,last)区间中所有的元素填充新值
