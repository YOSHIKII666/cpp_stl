...
{
   //key,value不允许重复
   //范型：key类型，value类型，hash函数，key的比较方式
   ...
   public:
      ...
      typedef typename base_type::mapped_type mapped_type;// value
      typedef typename base_type::value_type value_type;//key和value的组合类型，也就是那个pair
   ...
   public:
      ...
      unordered_map(InputIterator first,InputIterator last,
         const size_type buckrt_count=100,
         const Hash& hash=Hash(),
         const KeyEqual& equal=KeyEqual())
            :ht_(mystl::max(buckrt_count,static_cast<size_type>(mystl::distance(first,last))),hash,equal) {
      ...
      unordered_map(std::initializer_list<value_type> ilist,
         ...
         const Hash& hash=Hash(),
         const KeyEqual& equal=KeyEqual())
            :ht_(mystl::max(buckrt_count,static_cast<size_type>(ilist.size())),hash,equal) {
    ...
    :ht_(mystl::move(rhs.ht_))
      ...
      mapped_type& at(const key_type& key) {
         ...
         THROW_OUT_OF_RANGE_IF(it.node==nullptr,"the value is not exsist in unordered_map");
