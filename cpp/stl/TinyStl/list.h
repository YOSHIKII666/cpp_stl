...
namespace mystl
{
    ...
    template<class T>
    template<class... Args>
    typename list<T>::node_ptr list<T>::create_node(Args &&... agrs) {
        node_ptr p=node_allocator::allocate(1); //allocate，分配n*size_type个空间
        try {
            data_allocator::construct(mystl::address_of(p->value),mystl::forward<Args>(args)...);//(ptr,value)
            p->prev=nullptr;
            p->next=nullptr;
        }catch(...) {
